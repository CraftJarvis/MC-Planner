...
import src.utils.impala_lib.xf as xf
from src.utils.impala_lib.minecraft_util import store_args
from src.utils.impala_lib.tree_util import tree_map
...
def get_mask(first_b11: th.Tensor, state_mask: th.Tensor, t: int, T: int, maxlen: int, heads: int, device) -> th.Tensor:
    ...
    # FIXME: hacky! (see below)
    import copy
    prev_state_mask = copy.deepcopy(state_mask).zero_()
    state_mask = state_mask[:, :, 0].unsqueeze(1).bool()
    ...
    # FIXME: hacky! (see below)
    prev_state_mask.to(state_mask)
    prev_state_mask[:, :, 0] = state_mask.squeeze(1)
    state_mask = prev_state_mask
...
class MaskedAttention(nn.Module):
    ...
    def initial_state(self, batchsize: int, device=None):
        ...
        # FIXME: hacky! (see above)
        import copy
        state_mask = copy.deepcopy(state[0]).zero_()
        # Don't know why but you should never assign any non-zero value here
        # state_mask[:, :, 1] = -0.5
    ...
    def forward(self, input_bte, first_bt, state):
        ...
        if self.mask == "clipped_causal":
            ...
            if attemp != 0:
                print('Tried {} times'.format(attemp))

        # FIXME: hacky! (see above)
        state_mask = state_mask.to(xf_state[0].device)

        self.orc_block != new_mask
