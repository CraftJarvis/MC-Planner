...
import src.utils.impala_lib.torch_util as tu
from src.utils.impala_lib.masked_attention import MaskedAttention
    ...
    def initial_state(self, batchsize):
        if "lstm" in self.recurrence_type:
            ...
            for i in range(len(ret)):
                n_ret.append(ret[i][0].squeeze(0))
                n_ret.append(ret[i][1][0].squeeze(0))
                n_ret.append(ret[i][1][1].squeeze(0))
            return n_ret
