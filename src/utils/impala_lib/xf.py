...
from src.utils.impala_lib import misc, mlp
from src.utils.impala_lib import torch_util as tu
from src.utils.impala_lib import util
...
class SelfAttentionLayer(AttentionLayerBase):
    ...
    def initial_state(self, batchsize, initial_T=0):
        linear = self.q_layer.weight
        return (
            linear.new(batchsize, initial_T, self.x_size).to(self.dtype).zero_(),
            linear.new(batchsize, initial_T, self.x_size).to(self.dtype).zero_(),
        )
